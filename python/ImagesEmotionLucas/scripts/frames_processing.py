...
def PlotEmotionsVariation(scores=None):
    ...
    if scores:
        ...
        plt.gca().legend(('Anger', 'Disgust', 'Fear',
...
class ImageToEmotion:
    ...
    def __init__(self, heat_map=False, want_plot=False, webcam=False):
        ...
        self.webcam = webcam
        self.facial_emotion_recognition = FacialEmotionRecognition()
    ...
    def process_image(self, image):
        ...
        return self.facial_emotion_recognition.predict_frame(frame=image, heat_map=self.heat_map)
    ...
    def FolderImages(self):
        ...
        for file_name in files_names:
            ...
            ax = fig.add_subplot(
                int(np.round(len(files_names)/4)), 4, cont_files+1)

            # Llamamos a la clase de reconocimiento de emoción facial
            score, predicted_class = FER.predict_frame(
                frame=frame, heat_map=self.heat_map)
